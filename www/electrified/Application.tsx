...
export class Application extends AsyncConstructableClass{
    ...
    FEATURE_RAM_CONFLICT_DIALOG = false; // In development
    ...
    async constructAsync() {
        ...
        Ext.onReady(async () => {
            this.workspace = Ext.create('PVE.StdWorkspace');
        });
    ...
    _deleteItem(dataItem: any) {
        spawnWithErrorHandling(async () => {
            ...
            if(item instanceof Guest) {
                const result = await showBlueprintDialog({title: t`Delete ${item.ui_toString(false)}`, icon: "trash", niceElectrifiedStyle: false},(props) => {
    ...
    _createResourceTreeCellComponent(treeColumn: TreeColumn) {
         ...
         * @constructor
         */
        const OuterComponent = watchedComponent((props: any) => {
     ...
     */
    _startGuestInteractively(nodeName: string, type: string, guestId: number) {
        spawnWithErrorHandling(async () => {

            if(!this._datacenter) { // Not yet initialized
                // Just start the guest:
                await this.api2fetch("POST",`/nodes/${nodeName}/${type}/${guestId}/status/start`);
                return;
            }
