...
export abstract class Guest extends ModelBase implements NotificationTarget {
    ...
    _getImprovedStatus(plainValue: string | undefined) {
        ...
        if(tasks.some(task => task.type.endsWith("stop") && task.finishedSuccessful && task.endtime && task.endtime.getTime() + 1500 > now && !(this.lastStatusAction && this.lastStatusAction.timestamp > task.endtime?.getTime()) )) { // A stop task finished not long ago (and not other button was pressed in the meanwhile) ?
        ...
        if(this.type === "lxc" || !start) {
            try {
                ...
                if(toError(e).message.match(/zfs error: cannot set property for '.*': use 'none' to disable quota\/refquota/)) {
                    ...
                    if(start) {
                        await this.start(); // The above did not start it successful
                    }
                     // Ignore error
                }
                else {
